...
public class TileManager {
    ...
    public void render(Graphics g) {
        ...
        int xEnd = (int) Math.min(room.getWidth(), (room.getState().getCamera().getXOffset() +  room.getState().getGame().getWidth()) / Tile.TILE_WIDTH + 1);
        int yStart = (int) Math.max(0, room.getState().getCamera().getYOffset() / Tile.TILE_HEIGHT);
        int yEnd = (int) Math.min(room.getHeight(), (room.getState().getCamera().getYOffset() + room.getState().getGame().getHeight()) / Tile.TILE_HEIGHT + 1);
